...
%% initialize etc...
    ...
    if isempty(hIn.NXTMAC) 
        [DevHandle SerialNo]  = libusb_find_nxt_device();
    else 
        [DevHandle SerialNo] = libusb_find_nxt_device(hIn.NXTMAC);
    end
...
%% NXT found, open connection
    ...
    ret = libusb_set_configuration(DevHandle, LIBUSB_Configuration);
    ...
    ret = libusb_claim_interface(DevHandle, LIBUSB_Interface);
    displayLibusbStatus(ret);
    if (ret < 0)
        ErrorWhileOpening = true;
    end%if
    ...
    if ErrorWhileOpening || (DevHandle == 0)
        if ~SuppressErrors
...
function h_out = StartMotorControl(progname, h_in)
    ...
    if active
        ...
        while(true)
            ...
            if (matlab_toc(hTic) > reactionTimeout)
